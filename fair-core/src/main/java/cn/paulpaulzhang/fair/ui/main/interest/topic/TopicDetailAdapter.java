...
import cn.paulpaulzhang.fair.database.entity.TopicLikeCache;
import cn.paulpaulzhang.fair.database.entity.TopicLikeCache_;
import cn.paulpaulzhang.fair.database.entity.TopicPostCache;
import cn.paulpaulzhang.fair.database.entity.TopicUserCache;
 ...
 * 描述:
 */
public class TopicDetailAdapter extends BaseMultiItemQuickAdapter<TopicDetailItem, BaseViewHolder> {
    /**
     ...
     */
    TopicDetailAdapter(List<TopicDetailItem> data) {
        super(data);
        addItemType(TopicDetailItem.DYNAMIC, R.layout.view_dynamic_item);
        addItemType(TopicDetailItem.ARTICLE, R.layout.view_article_item);
    }

    @Override
    protected void convert(BaseViewHolder helper, TopicDetailItem item) {
        if (item.getItemType() == TopicDetailItem.DYNAMIC) {
        ...
        } else if (item.getItemType() == TopicDetailItem.ARTICLE) {
            TopicPostCache topicPostCache = item.getTopicPostCache();
            long id = topicPostCache.getId();
            long uid = topicPostCache.getUid();
            Box<LocalUser> localUserBox = ObjectBox.get().boxFor(LocalUser.class);
