...
public class DiscoveryAdapter extends BaseMultiItemQuickAdapter<Discovery, BaseViewHolder> {
    ...
    @Override
    protected void convert(@NotNull BaseViewHolder helper, Discovery item) {
        if (item.getItemType() == Discovery.DYNAMIC) {
            DiscoveryPostCache postCache = item.getPostCache();
            ...
            DiscoveryUserCache userCache = userBox.get(uid);
            ...
            mAdapter.setOnItemClickListener((adapter, view, position) -> {
                RecommendUser user = (RecommendUser) adapter.getItem(position);
                if (user != null) {
                    Intent intent = new Intent(mContext, UserCenterActivity.class);
                    intent.putExtra("uid", user.getUserCache().getId());
                    mContext.startActivity(intent);
                }
            });
        }
